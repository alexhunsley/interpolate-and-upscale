...
usage() {
  cat >&2 <<'EOF'
Usage:
  vidpipe.zsh [--interp N] [--width W] [--height H] [--scale S] [--keep] [--output_dir DIR] <input_video>
...
Options:
  ...
  --output_dir DIR  Output directory (defaults to input file's directory).
EOF
  exit 2
}
...
frames_dir="${out_dir}/frames_${stem}"
interp_dir="${out_dir}/frames_done_${stem}"
